   ...
   Checkboxes.prototype = {
      ...
      */
      ...
      updateSelect: function(selector, isSelected){
         //CUSTOM MODIFICATION BY TicketVault
         //Removes row with zero index as it is pinned row from selection
         if(selector.shift) selector.shift();
         var self = this;
         var dt = self.s.dt;
         var ctx = self.s.ctx;
...
// This IS CUSTOM code for TicketVault. Please DO NOT update file
	setTimeout(()=>{
         	// Select group checkbox based on "Select all" checkbox state
	       	$('.group-checkbox').prop('checked', ctrl.checked);
      	}, 0);
         // Prevent click event from propagating to parent
         e.stopPropagation();
      },
